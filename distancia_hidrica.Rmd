...
knitr::opts_chunk$set(echo = TRUE, warning = F, message = F, fig.align = 'center', class.source = "bg-info")
...
```{r plot_poligono_intro, echo=F, fig.width=10.37, fig.height=8}
...
grid <- makegrid(rio, cellsize = 0.0005) # Tamanho da célulalula em unidades de mapa!

# O grid Ã© um data.frame. para transformar em um  conjunto de dados espaciais utiliza-se a funÃ§ao SpatialPoints
grid <- SpatialPoints(grid, proj4string = CRS(proj4string(rio)))

# Extrair somente a parte do grid que intersepta o poligono do rio
grid <- grid[rio, ]

# Converte o grid novamente em um data.frame para criaÃ§Ã£o do raster
grid_df <- as.data.frame(grid)

# Atribui um valor para as cÃ©lulas que estÃ£o no rio (Ã¡gua)
grid_df$value <- 1
...
# Armazena os limites da Ã¡rea do rio para rosa dos ventos 
xmin <- extent(rio)[1]
xmax <- extent(rio)[2]
ymin <- extent(rio)[3]
ymax <- extent(rio)[4]
